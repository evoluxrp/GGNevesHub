...
local FolderName = "GG Neves Hub | Farm"
...
-- ========================================
-- TEMA VERDE-LIMÃO
-- Varre CoreGui + PlayerGui sem filtro de nome
-- Usa DescendantAdded para pegar novos elementos
-- ========================================
local LIME        = Color3.fromRGB(163, 230, 53)
local LIME_DARK   = Color3.fromRGB(77,  124, 15)
local LIME_MID    = Color3.fromRGB(101, 163, 13)
local BG_MAIN     = Color3.fromRGB(8,   12,  8)
local BG_SIDE     = Color3.fromRGB(13,  20,  13)
local BG_ITEM     = Color3.fromRGB(20,  32,  20)
local BG_TOGGLE   = Color3.fromRGB(30,  50,  30)
local TXT_MAIN    = Color3.fromRGB(230, 255, 220)
local TXT_MUTED   = Color3.fromRGB(120, 160, 120)

-- Cores originais da lib que vamos substituir
-- (preto puro, cinza escuro, azul de destaque padrão ZynLib)
local IS_DARK_BG = function(c)
    return c.R < 0.15 and c.G < 0.15 and c.B < 0.15  -- preto/quase preto
end
local IS_GREY_BG = function(c)
    local r,g,b = c.R, c.G, c.B
    return r > 0.08 and r < 0.22 and math.abs(r-g) < 0.03 and math.abs(g-b) < 0.03
end
local IS_BLUE_ACCENT = function(c)
    return c.B > 0.4 and c.B > c.R * 1.5 and c.B > c.G * 1.2
end
local IS_WHITE_TEXT = function(c)
    return c.R > 0.85 and c.G > 0.85 and c.B > 0.85
end
local IS_GREY_TEXT = function(c)
    local r,g,b = c.R,c.G,c.B
    return r > 0.4 and r < 0.8 and math.abs(r-g) < 0.05 and math.abs(g-b) < 0.05
end

local painted = {}  -- evita repintar infinitamente

local function PaintOne(v)
    if not v or not v.Parent then return end
    local id = tostring(v)
    if painted[id] then return end
    ...
    pcall(function()
        if cn == "Frame" or cn == "ScrollingFrame" then
            ...
            if v.BackgroundTransparency >= 1 then return end
            if IS_DARK_BG(c) then
                v.BackgroundColor3 = BG_MAIN
            elseif IS_GREY_BG(c) then
                v.BackgroundColor3 = BG_SIDE
            end

        elseif cn == "TextButton" then
            local c = v.BackgroundColor3
            if v.BackgroundTransparency < 1 then
                if IS_DARK_BG(c) or IS_GREY_BG(c) then
                    v.BackgroundColor3 = BG_ITEM
                elseif IS_BLUE_ACCENT(c) then
                    v.BackgroundColor3 = LIME_DARK
                end
            end
            if IS_WHITE_TEXT(v.TextColor3) then
                v.TextColor3 = TXT_MAIN
            elseif IS_GREY_TEXT(v.TextColor3) then
                v.TextColor3 = TXT_MUTED
            elseif IS_BLUE_ACCENT(v.TextColor3) then
                v.TextColor3 = LIME
            end

        elseif cn == "TextLabel" then
            if IS_WHITE_TEXT(v.TextColor3) then
                v.TextColor3 = TXT_MAIN
            elseif IS_GREY_TEXT(v.TextColor3) then
                v.TextColor3 = TXT_MUTED
        ...
        elseif cn == "TextBox" then
            local c = v.BackgroundColor3
            if v.BackgroundTransparency < 1 and (IS_DARK_BG(c) or IS_GREY_BG(c)) then
                v.BackgroundColor3 = BG_ITEM
            end
            if IS_WHITE_TEXT(v.TextColor3) then v.TextColor3 = TXT_MAIN end

        elseif cn == "ImageLabel" or cn == "ImageButton" then
            -- ícones azuis → verde
            if IS_BLUE_ACCENT(v.ImageColor3) then
                v.ImageColor3 = LIME
            end

        elseif cn == "UIStroke" then
            if IS_BLUE_ACCENT(v.Color) or IS_GREY_BG(v.Color) then
                v.Color = LIME_DARK
            end

        elseif cn == "UIGradient" then
            -- substitui gradiente azul por verde
            local hasBlue = false
            for _, kp in ipairs(v.Color.Keypoints) do
                if IS_BLUE_ACCENT(kp.Value) then hasBlue = true break end
            end
            if hasBlue then
                v.Color = ColorSequence.new({
                    ColorSequenceKeypoint.new(0, LIME_MID),
                    ColorSequenceKeypoint.new(1, LIME_DARK),
                })
            end
        end
    end)
...
-- Aplica nas duas raízes de GUI do Roblox
local function ApplyToAll()
    pcall(function() PaintAll(game:GetService("CoreGui")) end)
    pcall(function() PaintAll(game:GetService("Players").LocalPlayer.PlayerGui) end)
end

-- Primeira passagem logo após a janela criar
task.delay(0.3, ApplyToAll)
task.delay(1.0, ApplyToAll)
task.delay(3.0, ApplyToAll)

-- Monitora novos elementos adicionados em tempo real
pcall(function()
    game:GetService("CoreGui").DescendantAdded:Connect(function(v)
        task.delay(0.05, function() PaintOne(v) end)
    end)
end)
pcall(function()
...
-- ========================================
-- GRID 2x2 NA SIDEBAR DE TABS
-- Detecta o container real pelos filhos TextButton
-- ========================================
task.delay(1.5, function()
    pcall(function()
        local function FindTabHolder(root)
            for _, obj in ipairs(root:GetDescendants()) do
                if (obj:IsA("Frame") or obj:IsA("ScrollingFrame")) then
                    local btns = 0
                    for _, c in ipairs(obj:GetChildren()) do
                        if c:IsA("TextButton") or c:IsA("Frame") then btns = btns + 1 end
                    end
                    -- sidebar de tabs tem entre 10 e 20 filhos diretos
                    if btns >= 8 and btns <= 25 then
                        return obj
                    end
                end
            end
        ...
        local tabHolder = FindTabHolder(game:GetService("CoreGui"))
            or FindTabHolder(game:GetService("Players").LocalPlayer.PlayerGui)

        if not tabHolder then return end

        -- Remove UIListLayout existente
        local list = tabHolder:FindFirstChildOfClass("UIListLayout")
        if list then list:Destroy() end

        -- Cria UIGridLayout 2 colunas
        local grid = tabHolder:FindFirstChildOfClass("UIGridLayout")
        if not grid then
            grid = Instance.new("UIGridLayout")
            grid.Parent = tabHolder
        end
        grid.CellSize        = UDim2.new(0.5, -3, 0, 54)
        ...
        -- Estiliza cada tile de tab
        for i, child in ipairs(tabHolder:GetChildren()) do
            if child:IsA("TextButton") or (child:IsA("Frame") and child.Name ~= "UIGridLayout") then
                child.BorderSizePixel = 0
                child.BackgroundColor3 = BG_ITEM

                local corner = child:FindFirstChildOfClass("UICorner")
                    or Instance.new("UICorner", child)
                corner.CornerRadius = UDim.new(0, 8)

                local stroke = child:FindFirstChildOfClass("UIStroke")
                    or Instance.new("UIStroke", child)
                stroke.Color = LIME_DARK
                stroke.Thickness = 1

                -- reaplicar texto dos labels internos
                for _, lbl in ipairs(child:GetDescendants()) do
                    if lbl:IsA("TextLabel") then
                        lbl.TextColor3 = TXT_MUTED
                        lbl.TextSize   = 10
                        lbl.Font       = Enum.Font.GothamBold
                    end
                    if lbl:IsA("ImageLabel") then
                        lbl.ImageColor3 = LIME
                    end
                end
                ...
                if child:IsA("TextButton") then
                    child.MouseEnter:Connect(function()
                        child.BackgroundColor3 = Color3.fromRGB(26, 47, 20)
                        local s = child:FindFirstChildOfClass("UIStroke")
                        if s then s.Color = LIME_MID end
                    end)
                    child.MouseLeave:Connect(function()
                        child.BackgroundColor3 = BG_ITEM
                        local s = child:FindFirstChildOfClass("UIStroke")
                        if s then s.Color = LIME_DARK end
                    end)
                end
            end
        end
...
-- =========================================================
-- AUTO FISHING SYSTEM (Ported from GG Neves Hub)
-- =========================================================
        ...
        pcall(function()
            while wait(Sec) do
                if _G.Auto_Rainbow_Haki then
                    -- Copy logic Rainbow Haki từ file GG Neves Hub.txt (dòng 2058-2154) vào đây nếu cần
